import { FileStack, RefreshCw, Sheet, Sparkles } from 'lucide-react'
import type {
  GoogleWorkspaceData,
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.075] p-4 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-400">
            ดึงข้อมูลจาก Apps Script Web App API เมื่อเชื่อมต่อ URL แล้ว.
          </p>
          <button
        ...
        {status === 'error' ? (
          <p className="mt-4 rounded-2xl border border-[#ba5835]/30 bg-[#ba5835]/12 p-4 text-sm text-[#ffb08d]">
            เชื่อมต่อ Google Workspace ไม่สำเร็จ ตรวจสอบ Web App URL และสิทธิ์
        ...
        {status === 'empty' ? (
          <p className="mt-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-sm text-slate-400">
            เชื่อมต่อแล้ว แต่ยังไม่มีข้อมูลล่าสุดจาก Apps Script.
          </p>
        ) : null}
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.045] p-4">
          ...
          {data?.latestUpdates?.length ? (
            ...
            <p className="mt-2 text-sm text-slate-500">ยังไม่มีอัปเดตล่าสุด</p>
          )}
        </div>
      </div>
    </section>
